...
| --- | --- | --- |
| `run_size.py` | Trains a ResNet50 on each of the eleven preprocessing configurations and scores it on OCTDL and the clinic. | `results/size_results.csv` |
...
`run_size.py`, `run_models.py` and `eval_checkpoints.py` are resumable. A row that
...
Local training on Apple silicon is memory bound, so the size search can be run on
