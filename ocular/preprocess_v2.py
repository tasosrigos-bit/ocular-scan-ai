...
knob is illustrated in ``notebooks/02_preprocessing.ipynb``.
...
#: Pixel depth blanked inward from each corner to remove device UI icons.
#: Measured from the OPTOPOL clinic exports: the icons sit a near-constant
#: number of pixels from the edge regardless of the export's resolution (they
#: are a fixed-size overlay burned in before export), so an absolute-pixel
#: margin covers them across export sizes better than a margin that scales
#: with the image, which either misses larger exports or over-blanks tightly
#: cropped ones.
CORNER_MARGIN_PX_W = 140
CORNER_MARGIN_PX_H = 90
#: Cap on the corner margin as a fraction of width/height, so the absolute
#: pixel margin above cannot blank a large chunk of an unexpectedly small image.
CORNER_MARGIN_FRAC_CAP = 0.40
...
def blank_corners(
    g: np.ndarray,
    margin_w: int = CORNER_MARGIN_PX_W,
    margin_h: int = CORNER_MARGIN_PX_H,
    frac_cap: float = CORNER_MARGIN_FRAC_CAP,
) -> np.ndarray:
    """Replace a fixed-pixel rectangle at each corner with the background level.

    Some devices burn UI icons (scale bars, laterality markers, scan-direction
    arrows) into the frame a fixed number of pixels inset from the border,
    regardless of the export's resolution. :func:`padding_mask` only removes
    near-white regions connected to the border, so an inset icon that is not
    near-white survives it untouched and can still be picked up by band
    detection or by the model itself. This blanks a fixed-pixel corner
    rectangle before anything else runs, which is a coarser cut than
    :func:`padding_mask` but does not depend on the icon's brightness or on it
    touching the edge. The margin is expressed in pixels rather than as a
    fraction of the image because the icon's distance from the edge is
    constant across export sizes, while the fraction of the frame occupied by
    real retinal tissue is not: a fraction large enough to clear the icon on a
    wide export over-crops a tightly framed one.

    Parameters
    ----------
    g : numpy.ndarray
        Grey-scale image of shape ``(H, W)``.
    margin_w, margin_h : int, optional
        Pixel depth blanked inward from the left/right and top/bottom edges
        at each of the four corners.
    frac_cap : float, optional
        Upper bound on the margin, as a fraction of the width or height, so
        an unexpectedly small image is not mostly blanked out.

    Returns
    -------
    numpy.ndarray
        Copy of ``g`` with each corner rectangle set to the background level.
    """
    g = g.copy()
    h, w = g.shape
    mh = min(margin_h, int(frac_cap * h))
    mw = min(margin_w, int(frac_cap * w))
